...
var REGISTRY = [
  ...
  { id: "backend-developer", category: "coding", triggers: ["backend", "rest api", "graphql", "microservice", "payment ledger", "ledger api", "node", "express", "fastapi", "django", "laravel", "flask", "spring boot", "rails", "nestjs"] },
  { id: "database-optimizer", category: "data", triggers: ["database", "postgres", "sql", "schema migration", "database migration", "query optimization", "database index"] },
  { id: "data-engineer", category: "data", triggers: ["etl", "data pipeline", "data warehouse", "data ingestion", "data lake", "batch processing", "stream processing"] },
...
function normalizeTools(raw) {
  ...
  return String(raw).split(",").map((t) => t.trim()).filter((t) => t.length > 0);
}
...
async function scanDirs(dirs, agents = /* @__PURE__ */ new Map()) {
  ...
  for (const root of roots) {
    ...
    for (const plugin of pluginNames) {
      ...
      for (const fileName of fileNames) {
        ...
        const description = flattenDescription(fm["description"]);
        const modelRaw = fm["model"];
        const model = typeof modelRaw === "string" && modelRaw.trim() && modelRaw.trim() !== "inherit" ? modelRaw.trim() : void 0;
        const agent = {
          name,
          description,
...
async function scanCategoryRoots(roots, agents) {
  for (const root of roots) {
    let categoryNames;
    try {
...
async function scanSkillDirs(skillsDir, root) {
  let dirNames;
  try {
...
async function scanCommandFiles(commandsDir, root) {
  let fileNames;
  try {
...
function serializeToolkits(toolkits) {
  const out = {};
...
async function loadExisting(outDir) {
  ...
  const rootsRaw = parsed["roots"];
  const roots = Array.isArray(rootsRaw) ? rootsRaw.filter((r) => typeof r === "string") : [];
...
async function fileStillExists(file, roots) {
...
function makeEntry(scanned) {
  ...
  if (scanned.plugin !== void 0) entry["plugin"] = scanned.plugin;
  if (scanned.model !== void 0) entry.model = scanned.model;
  return entry;
}
function resolveDomainAlias(domain, existingDomains) {
  ...
  const matches = existingDomains.filter(
    (d) => d.startsWith(`${domain}-`) || d.endsWith(`-${domain}`) || d.includes(`-${domain}-`)
  );
  const only = matches[0];
  return matches.length === 1 && only !== void 0 ? only : domain;
}
function merge2(scanned, existing) {
...
function serializeIndex(buckets, roots) {
  const domains = {};
...
async function runScan(argv) {
  ...
  if (existing) {
    ...
    for (const name of existing.assignments.keys()) {
      ...
      if (entry.model !== void 0) revived.model = entry.model;
      scanned.set(name, revived);
    }
  }
  const result = merge2(scanned, existing);
  const changedFiles = [];
  const staleRemoved = [];
  ...
  if (opts.check) {
    lines.push(
      changedFiles.length > 0 ? `[check] dry run \u2014 would write: ${changedFiles.join(", ")}` : "[check] dry run \u2014 no changes"
    );
  } else {
    lines.push(
