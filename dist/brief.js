...
// src/hydrate.ts
var DEFAULT_TEMPLATE = `You are operating under an adopted specialist role from a plugin marketplace.
...
async function effectiveRoots(mapDir) {
  const index = await readJson(path.join(mapDir, "index.json"));
  const roots = Array.isArray(index?.roots) ? [...index?.roots] : [];
...
async function loadPluginToolkit(resolved) {
  let skills = [];
  let commands = [];
  let block = "";
...
async function hydrateAgent(agentName, task, opts = {}) {
