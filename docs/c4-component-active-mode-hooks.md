...
- **PreToolUse contract gate**: On every `Edit`/`Write`/`MultiEdit`/`NotebookEdit` targeting a file inside the active repo, denies the edit until a fresh routing decision exists at the session-scoped `~/.invoker/state/promper-decision-<session_id>.json` (any verdict, same repo root, 60-min TTL; the legacy global `promper-decision.json` is accepted as a permanent fallback for environments without a session_id). The deny reason re-delivers the contract summary and the exact JSON to record. Out-of-repo writes (including the state file itself) are never gated; every unexpected condition fails open.
- **SessionEnd gate re-arm**: Clears this session's routing decision when the session ends, so the next session's first repo edit re-runs the agent-walk. Session-scoped — never wipes another live session's decision (same repo or not); also sweeps session decision files older than the TTL. Without a session_id, falls back to the legacy repo-scoped delete on the global file.
...
- **Shared state file** (session-scoped `~/.invoker/state/promper-decision-<session_id>.json`, legacy global `promper-decision.json` as fallback, on the local filesystem): the hand-off bridge between `gate-prompt.mjs` and `enrich-spawn.mjs`. Written when the user/model acts on the NUDGE (records `verdict`/`agent`/`reason`/`ts`); read by `enrich-spawn.mjs` (via `buildBrief()`'s `statePath` option, preferring the session file when it exists) to recover which role/persona a subagent spawn should inherit. The two hooks never call each other directly — they are decoupled entirely through this file.
